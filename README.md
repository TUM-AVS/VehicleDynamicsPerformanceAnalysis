...
- Reads CSV, MATLAB MAT, ROS 2 MCAP, and candump files.
...
- Processing stages:
   ...
   - Characterization of cornering segments (braking/entry/mid/exit/straight, low/mid/high speed)
...
- Optionally creates a Run Report PDF and launches the Plot Creator GUI.
...
[`TAM__common`](https://github.com/TUMFTM/TAM__common) dependency to a proven commit. From this repository's root,
import that revision into the parent directory and initialize its pinned submodules recursively:
...
> [!WARNING]
> `config/vehicle_handler/GenericVehicle` is synthetic. Its
...
For real logs, provide the actual vehicle configuration containing both
...
contains three generic laps using the standard ros msgs type `std_msgs/msg/Float64`. Generation details are in the [synthetic example guide](examples/README.md).
...
2. Choose `GenericVehicle` and leave **Estimate instantaneous cornering
   ...
   parser profile is selected automatically during parsing.
...
A unified CSV contains source channels renamed to common names and units for whichever channels the parser profile offers a configuration. Other source channels without a defined name and unit conversion are retained unchanged. Because the later derived channel calculations are conditional, different parser profiles
...
| `.mcap` | ROS 2 MCAP. If your have custom msgs types, make sure they are sourced. Only topics in [`config/topic_list.csv`](config/topic_list.csv) are read and flattened and the rest is skipped during the parsing stage. |
...
when all of its non-empty source names exist. Smoothing windows length in seconds for the forward-backward moving average signal smoothing are
...
7. Slice data into laps.
8. Smooth configured signals.
9. Characterize corner phases and, if enabled, estimate front and rear instantaneous
   cornering stiffness.
10. Write the complete converted log and its per-lap CSV files.

If neither `s_m` nor `s_norm_m` distance coordinate is available a GUI windows opens to draw a finish line. This will get used to differentiate the distance and lap signal. However, this is the distance of the driven path and can create `s_m` of varying length so if a reference coordinate exists supplying this via the parser config is preferred. 
The pipeline requires at least `time_s` and a usable `s_m` or `s_norm_m` distance coordinate before it can write per-lap files.
...
mapped to this reference and written as `s_norm_m` via a K-Nearest-Neighbour estimation.
...
This is an overview of the derived signal libary. All channels are conditional, which means that before they get calculated there is a check if the intended output signal does not exists and if the required input channels are available.
So it can derive signals which are not existing in the logs but prefers signals if they are directly supplied through the parser. Wheel suffixes use `fl`, `fr`, `rl`, and `rr` for
front-left, front-right, rear-left, and rear-right. Vehicle parameters refer to the selected vehicle and tire configuration.
...
| `curvature`, `curvature_smoothed` | `radius_m` | Signed inverse turn radius in `1/m` |
...
| `alpha_f_rad`, `alpha_r_rad` | `alpha_fl_rad`, `alpha_fr_rad`, `alpha_rl_rad`, `alpha_rr_rad` | Left/right mean for each axle. |
...
| `whl_fl_radius_m`, `whl_fr_radius_m`, `whl_rl_radius_m`, `whl_rr_radius_m`, `whl_f_radius_m`, `whl_r_radius_m` | `v_mps`, all four wheel angular speeds | Zero-slip wheel-radius estimates `v_mps / wheel_radps` at or above `3 m/s`, plus front- and rear-axle means. |
...
| `fz_f_N`, `fz_r_N`, `fz_fl_N`, `fz_fr_N`, `fz_rl_N`, `fz_rr_N` | `v_mps`, `ax_mps2`, `ay_mps2`, vehicle mass, geometry, and aerodynamic parameters | Estimated positive-downward axle and wheel loads in newtons, including static load, aerodynamic load, longitudinal and lateral load transfer. |
| `fy_f_norm_N`, `fy_r_norm_N`, `fy_norm_N` | Axle lateral and vertical loads | Dimensionless `Fy/Fz` ratios for each axle|
...
| Signal | Required inputs | Description |
| --- | --- | --- |
| `s_norm_m` | Reference log: `s_m`, `pos_x_m`, `pos_y_m`; later logs: `pos_x_m`, `pos_y_m` | Common distance coordinate. The first suitable log trains a three-neighbor KNN-model; later logs use that model. |
| `n_lap` | `s_norm_m` when available, otherwise `s_m` | One-based lap number. A new lap is detected at a distance drop greater than `300 m`, with resets separated by at least 100 samples. |
...
After a successful conversion, the application can generate a Run
...
The entered date and session are used in
...
than 10 seconds contain laps covering less than 5 percent of the longest valid lap. The resulting report contains run and performance
summaries, best and average lap times, sector comparisons, speed and
...
If the variable is unset or empty, weather lookup is skipped. When set, only the city name is sent to
...
slip angle. In the linear operating region it is the slope of the
lateral-force versus slip-angle relationship. Tracking and instantaneous estimate over
...
angles, and derived lateral forces. Results depend on signal
quality, vehicle parameters, and sufficient cornering excitation; treat them as informal estimates rather than direct measurements.

The estimator processes the complete log and can span lap boundaries. It applies a Butterworth filter, and uses dynamically sized left-sided windows with a default
minimum slip-angle interval of 0.02 rad. Although each fitting window uses current and past samples, filtering uses future samples, so this is
an offline estimate rather than a real-time estimator.
...
| `cs_ratio_f`, `cs_ratio_r` | Axle slip angles, axle lateral forces, and cornering-stiffness estimates | Estimated stiffness relative to a linear-region reference. A value near 1 represents linear stiffness; lower values indicate a reduced stiffness in the non-linear range. Zero or negative values indicate the tire force peak and the region behind it. |
...
Select the front or rear axle, a lap, and a distance location.
